...
func ModulePayroll(app *fiber.App, db *gorm.DB, dbSimpeg *gorm.DB) {
	group := app.Group("/api/payroll")
	...
	group.Get("/", func(c *fiber.Ctx) error {
		...
		bulanNum, errBulan := strconv.Atoi(bulanStr)
		if errBulan != nil {
			return c.JSON(fiber.Map{
				"status":  "fail",
		...
		namaBulanStr := NamaBulan(bulanNum)
		nip := c.Query("nip")
		...
		var slipGaji SlipGaji
		query := targetDB.Table("payroll_publishb").Where("tahun = ?", tahunStr).Where("bulan = ?", bulanNum)
		if nip != "" {
			query = query.Where("nip = ?", nip)
		}

		err := query.First(&slipGaji).Error
		if err != nil {
