...
export const CutiPage = () => {
  ...
  const fetchCutiList = async () => {
    ...
    try {
      ...
      showToast(err.message || 'Gagal memuat data pengajuan cuti', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDetail = (item) => {
    ...
    setCatatanSdm(item.catatan_atasan || item.catatan || '');
    setIsModalOpen(true);
  };
  ...
  const handleVerifyAction = async (newStatus) => {
    ...
    try {
      await apiClient.putForm(`/api/leave/${selectedCuti.id}`, {
        status: newStatus,
        catatan_atasan: catatanSdm.trim(),
        role: 'sdm',
      });

      const actionText = newStatus === 'terima sdm' ? 'disetujui' : 'ditolak';
      showToast(`Pengajuan cuti NIP ${selectedCuti.nip} berhasil ${actionText} oleh SDM!`, 'success');
      setIsModalOpen(false);
      fetchCutiList();
    } catch (err) {
      showToast(err.message || 'Gagal memproses verifikasi cuti', 'error');
    } finally {
  ...
  return (
    <div className="page-wrapper animate-fade-in">
      ...
      <div style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800 }}>Verifikasi Pengajuan Cuti</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          Tinjau dan setujui / tolak pengajuan cuti pegawai Universitas Pakuan (Khusus SDM)
        </p>
      </div>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="10" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
                  Tidak ada data pengajuan cuti pada kriteria filter ini.
                </td>
              </tr>
            ) : (
              paginatedList.map((item, index) => {
                ...
                return (
                  <tr key={item.id || index}>
                    ...
                    <td>
                      <span style={{ fontSize: '0.825rem', fontWeight: 600, color: '#0369a1' }}>
                        {item.nama_jenis_cuti || item.jenis_cuti || 'Cuti Tahunan'}
                      </span>
                    </td>
                    <td>
                    ...
                    <td>
                      <span style={{ fontWeight: 700, fontSize: '0.875rem' }}>{item.jumlah_hari || 1} Hari</span>
                    </td>
                    <td>
      ...
      {/* Modal: Tinjau & Verifikasi Cuti SDM */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title="Tinjau Pengajuan Cuti Pegawai"
        maxWidth="600px"
        footer={
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <h4 style={{ fontSize: '1rem', fontWeight: 800 }}>{selectedCuti.nama_pemohon || selectedCuti.nama || selectedCuti.nip}</h4>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '2px' }}>
                    NIP: {selectedCuti.nip}
                  </p>
                  <p style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '2px' }}>
                    {[selectedCuti.fakultas, selectedCuti.prodi, selectedCuti.unit].filter(Boolean).join(' • ') || 'Pegawai Unpak'}
                  </p>
                </div>
                <Badge status={selectedCuti.status} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Jenis Cuti</span>
                <p style={{ fontWeight: 600, fontSize: '0.875rem', color: '#0369a1' }}>
                  {selectedCuti.nama_jenis_cuti || selectedCuti.jenis_cuti || 'Cuti'}
                </p>
              </div>
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Jumlah Hari</span>
                <p style={{ fontWeight: 700, fontSize: '0.875rem' }}>{selectedCuti.jumlah_hari || 1} Hari Kerja</p>
              </div>
              <div style={{ gridColumn: 'span 2' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Periode Cuti</span>
                <p style={{ fontWeight: 700, fontSize: '0.875rem' }}>
                  {formatRentangTanggalIndo(selectedCuti.tanggal_mulai, selectedCuti.tanggal_selesai)}
                </p>
              </div>
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Verifikasi Atasan</span>
                <p style={{ fontWeight: 600, fontSize: '0.875rem' }}>
                  {selectedCuti.verifikasi ? `Disetujui NIP (${selectedCuti.verifikasi})` : 'Belum Diverifikasi'}
                </p>
              </div>
            </div>

            <div>
              <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Alasan Cuti</span>
              <p style={{ marginTop: '4px', fontSize: '0.875rem', backgroundColor: '#f8fafc', padding: '10px 12px', borderRadius: '8px' }}>
                {selectedCuti.alasan || '-'}
              </p>
            </div>

            {selectedCuti.file_lampiran && (
              <div>
                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-muted)' }}>Berkas Lampiran</span>
                <div style={{ marginTop: '4px' }}>
                  <a
                    href={selectedCuti.file_lampiran.startsWith('http') ? selectedCuti.file_lampiran : `${apiClient.baseUrl || ''}/${selectedCuti.file_lampiran}`}
                    target="_blank"
                    rel="noreferrer"
                    className="btn btn-secondary btn-sm"
